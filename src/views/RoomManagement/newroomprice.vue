<template>
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar color="purple">
        <ion-buttons slot="start">
          <ion-menu-button color="primary"></ion-menu-button>
        </ion-buttons>
        <ion-title>กำหนดค่าห้อง</ion-title>
      </ion-toolbar>
    </ion-header>

    <ion-content :fullscreen="true">
      <ion-header collapse="condense">
        <ion-toolbar>
          <ion-title size="large">{{ $route.params.id }}</ion-title>
        </ion-toolbar>
      </ion-header>

      <div id="container">
        <ion-card>
          <ion-item color="light">
            <ion-card-header>
              <ion-card-title>กำหนดราค่าห้อง</ion-card-title>
            </ion-card-header>
          </ion-item>

          <ion-card-content>
            <ion-list>
              <ion-item>
                <ion-label>ห้อง :</ion-label>
                <ion-input>{{ $route.params.room_id }}</ion-input>
              </ion-item>
            </ion-list>
          </ion-card-content>
        </ion-card>
        <ion-card>
          <ion-radio-group value="turtles">
            <ion-item color="light">
              <ion-card-title>บริการเพิ่มเติม</ion-card-title>
            </ion-item>
            <ion-item color="light">
              <div className="ion-float-end">
                <ion-button routerLink="/newroomsix">+ เพิ่มค่าบริการ</ion-button>
              </div>
            </ion-item>
            <ion-grid fixed="true">
              <ion-item>
                <ion-col>ค่าจอดรถ</ion-col>
                <ion-col>300</ion-col>
                <ion-col>
                  <ion-text color="tertiary" routerLink="">แก้ไข</ion-text>
                </ion-col>
              </ion-item>

              <ion-item>
                <ion-col>ค่าอินเตอร์เน็ต</ion-col>
                <ion-col>500</ion-col>
                <ion-col>
                  <ion-text color="tertiary" routerLink="">แก้ไข</ion-text>
                </ion-col>
              </ion-item>

              <ion-item>
                <ion-col>ค่าเฟอร์นิเจอร์</ion-col>
                <ion-col>500</ion-col>
                <ion-col>
                  <ion-text color="tertiary" routerLink="">แก้ไข</ion-text>
                </ion-col>
              </ion-item>
            </ion-grid>
          </ion-radio-group>
        </ion-card>

        <ion-card>
          <ion-radio-group>
            <ion-item color="light">
              <ion-card-title>สถานะห้อง</ion-card-title>
            </ion-item>
            <ion-item>
              <ion-label>ว่าง</ion-label>
              <ion-radio slot="end" value="dogs"></ion-radio>
            </ion-item>

            <ion-item>
              <ion-label>ไม่ว่าง</ion-label>
              <ion-radio slot="end" value="cats"></ion-radio>
            </ion-item>
          </ion-radio-group>
        </ion-card>


        <ion-card>
          <IonRow>
            <div className="ion-float-end">
              <ion-button routerLink="/newroomeOne">บันทึก</ion-button>
            </div>
          </IonRow>
        </ion-card>
      </div>
    </ion-content>
  </ion-page>
</template>

<script lang="ts">
import { defineComponent } from 'vue';
import { IonInput,IonRadioGroup, IonItem, IonCol, IonRow, IonCard, IonButtons, IonContent, IonHeader, IonMenuButton, IonPage, IonTitle, IonToolbar, IonLabel, IonButton } from '@ionic/vue';

export default defineComponent({
  name: 'FolderPage',
  components: {
    IonInput,IonRadioGroup, IonItem, IonCol, IonRow, IonCard, IonButtons, IonContent, IonHeader, IonMenuButton, IonPage, IonTitle, IonToolbar, IonLabel, IonButton
  }
});
</script>

<style>
.ion-color-purple {
  --ion-color-base: var(--ion-color-purple);
  --ion-color-base-rgb: var(--ion-color-purple-rgb);
  --ion-color-contrast: var(--ion-color-purple-contrast);
  --ion-color-contrast-rgb: var(--ion-color-purple-contrast-rgb);
  --ion-color-shade: var(--ion-color-purple-shade);
  --ion-color-tint: var(--ion-color-purple-tint);
}

ion-content {
  --background: #e7e6eb;
  --color: #fff;
}
</style>

